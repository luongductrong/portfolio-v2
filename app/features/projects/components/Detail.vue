...
<template>
  <section class="pb-20 pt-6 sm:pb-24 sm:pt-8" :aria-labelledby="`project-${props.project.slug}-title`">
    <LayoutContainer class="py-0">
      <NuxtLink
        to="/projects"
        class="mb-6 inline-flex min-h-11 items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"
      >
      ...
      <Motion as="div" :initial="{ opacity: 0, y: 20 }" :animate="{ opacity: 1, y: 0 }" :transition="{ duration: 0.6 }">
        ...
        <header
          class="relative z-10 mx-3 -mt-12 rounded-lg border bg-card p-6 shadow-xl sm:mx-8 sm:p-8 lg:mx-14 lg:p-10"
        >
